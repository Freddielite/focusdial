...
export async function initSchema() {
  await pool.query(`
    ...
    -- Auto-generated Reminders (see routes/reminders.js and the two new
    -- columns on reminders further down) for a deadline that's slipped
    -- to behind/overdue or a budget under 70% -- distinct from
    -- automation_deadline_pace above, which only fires a transient push
    -- notification. This one leaves a persistent Reminder the person
    -- still has to act on/dismiss, so it gets its own toggle rather than
    ...
    -- Marks a reminder as auto-generated by a risk rule (see
    -- routes/reminders.js's POST handler and the frontend's
    -- computeAutomationTriggers in analytics.js) rather than typed in by
    -- hand, and which deadline/budget it's about. The only thing this is
    -- actually for is the dedup check before creating a new one -- "is
    -- there already a pending reminder for this exact deadline/budget" --
    -- so a person doesn't get a fresh reminder every time the app loads
    -- while the same deadline stays behind pace. Both nullable: a normal,
    -- user-created reminder has neither set.
    ALTER TABLE reminders ADD COLUMN IF NOT EXISTS auto_source_type TEXT CHECK (auto_source_type IN ('deadline_risk', 'budget_risk'));
    ALTER TABLE reminders ADD COLUMN IF NOT EXISTS auto_source_id UUID;
    CREATE INDEX IF NOT EXISTS idx_reminders_auto_source ON reminders(auto_source_type, auto_source_id) WHERE auto_source_type IS NOT NULL;
