...
// sent/accepted cross-origin (frontend on Vercel, backend on Render) —
// origin: true (reflecting the request's actual Origin header) is
...
// Not session-protected — this is hit by an external scheduler (e.g.
// cron-job.org), not a logged-in browser, so it uses its own
// CRON_SECRET query-param check instead (see routes/cron.js).
