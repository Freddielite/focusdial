...
//
// Nothing is sent to the server until the window closes with no Undo - // there's no soft-delete/restore-on-the-backend involved, which is what
  ...
  function requestDelete({ id, label, onHide, onRestore, deleteFn, afterCommit }) {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        // The window closed and the delete itself failed server-side - // restore what was optimistically hidden rather than leaving the
